...
public interface GameObj {
	...
	default boolean isAbove(GameObj that) {
		return isAbove(that.pos().y);
	}

	default boolean isUnderneath(GameObj that) {
	...
	default boolean isLeftOf(GameObj that) {
		return isLeftOf(that.pos().x);
	}

	default boolean isRightOf(GameObj that) {
